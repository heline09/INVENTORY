...
def borrow_equipment(request, student_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            student_equipment = form.save(commit=False)
            student_equipment.borrow_equipment()
...
def search_students(request):
    ...
    if search_term:
        students = (
            Student.objects.filter(first_name__icontains=search_term) |
            Student.objects.filter(last_name__icontains=search_term) |
            Student.objects.filter(student_id__icontains=search_term)
        )
        students = students.values('id', 'first_name', 'last_name', 'student_id')  # Return only necessary fields
